...
public class UnblockCount implements SearchHeuristic {
	...
	public int calculate(State state) {
		...
		while (startingX < Constants.MAPSIZE) {
			movesToUnblock += getNumMovesToUnblock(map, 2, startingX, 2) + 1;
			startingX++;
		}
	...
	int getNumMovesToUnblock(Map map, int x, int y, int iterations) {
		...
		if (car.getDirection() == Constants.VERTICAL) {
			...
			if (y - car.getLength() < 0) {
				movesToUnblockAbove = -1;
			} else {
			...
			if (y + car.getLength() >= Constants.MAPSIZE) {
				movesToUnblockBelow = -1;
			} else {
			...
			if (x - car.getLength() < 0) {
				movesToUnblockLeft = -1;
			}
			else {
			...
			if (x + car.getLength() >= Constants.MAPSIZE) {
				movesToUnblockRight = -1;
			}
			else {
